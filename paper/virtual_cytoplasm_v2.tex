...
Diffusion coefficient of crowders from single molecules to cytoplasm are decreased in a range of 84\% (Ppa) to 93\% (tRNA) (Fig.~\ref{fig:translational_diffusion} A). The straight (fitted) line with the slope very close to zero  ($m = 0.00028$) shows that the drop in diffusion coefficient is independent of the size of proteins. The largest drop of diffusion coefficient corresponds to tRNA and we found that the aggregation of ATP$^{-3}$ and FBP$^{-4}$ around Mg$^{2+}$ which was used as cation for tRNA leads to its confinement. In fact, by removing diffusion coefficient of tRNA from the data points the slope of the line decreases by half ($m=0.00014$).
