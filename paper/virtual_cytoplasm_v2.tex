\documentclass[journal=jpcisd8,manuscript=article]{achemso}
% \linespread{1.5}
...
of macromolecules when compared to the dilute environment  in
experiments on isolated proteins. Computational modeling and
...
all-atom explicit solvent models of the prokaryote cytoplasm has now become
...
simulations. 
The model consists of 8 different proteins representing about 50\% of the cytoplasmic proteins and one type of t-RNA molecule. 
...
work we provide a Python script and a set of files to enable other
...
leading to spatial constraints that directly affects biological
activity~\citep{ostrowska2019}. This effect, called macromolecular
...
as a result of the entropic component of the Gibbs energy, given by the excluded volume
...
\textit{in vitro}, the understanding of the influence of crowded
environments \textit{in vivo} is still evolving.  Techniques such as
...
among the most important to the field due to their ability to probe
cells in a non destructive way. 

Another powerful class of methods is computational modeling and simulation.
As the available computational power increases, models become
increasingly complex yielding better representations of reality. The
 simplest class of computational models applies Brownian Dynamics
...
characterization of dynamical effects that  take place in such a
large system. It was estimated, by extrapolating the advances in
...
calculated at the HF/6-31G* theory level with Gaussian16~\cite{g16}.  Properties
...
Droplets containing each component of the model surronded by water and counter-ions were inserted in an empty box in random positions and orientations. Since the volume of this box  initially was larger than  strictly necessary to accomodate all components, the system was allowed to shrink during a short simulation. Details about how the simulation box was built and optimized are in the results section. All
...
in a water box, had a length of 200 ns. The cytoplasm models were run
for 1 \SI{}{\micro\second}. All systems were simulated in three
replicas.  The replicas for the dilute systems were generated by
different initial velocities, but each replica of the cytoplasm model was taken to be a
...
Analyses were done using GROMACS tools~\cite{Lindahl2001a,Spoel2005a,Hess200a} after removing artifacts
...
coefficients need to be corrected for finite size
...
our model, which consists of five fractions: protein, RNA,
metabolites, water and ions. It should be noted that we did not add
lipids and DNA to our model because we consider only elements that are
free to diffuse through the cytosol. We gathered data from several
sources in order to build a computational model that is representative
of the cytoplasm of {\em Escheria
  coli}~\cite{Dong1996,Bennett2009,Link1997,Mcguffee2010}.  It has
been estimated that a {\em E. coli} cell contains about 50,000
ribosomes per $\mu^3$~\cite{Milo2017a}. The volume of our box is about
12,000 nm$^3$ (\figref{shrink}), therefore in principle one (0.6 to be
precise) ribosome molecule should be added, however to reduce the
...
each protein was kept as reported in their respective crystallographic structure
...
molecules that were added to the cytoplasm model (\tabref{tbl:soup_components}).

\subsubsection{Metabolites fraction}
The most abundant molecule of each metabolite class was used to 
...
for each molecule (\tabref{tbl:soup_components}) was calculated from the ratios between their
...
such as the \textit{E. coli} cytoplasm~\cite{Zimmerman1991}, the highest concentration corresponds to volume occupation as high as 40\%~\cite{Ellis2003a}. In our case, we
choose a biomolecular concentration of 30\%, that is, the number of water
...
simulation box and to reach the ionic strength of 150 mmol/L by
...
All components were put in the same simulation box by inserting each
of them in random orientations in a cubic volume of side $L$ that is
initially empty. However, that is not a trivial process. It is necessary  to
...
interactions between the elements by placing them too close to each
...
in which each component was equilibrated previously. The benefits of
...
which will speed up the equilibration of the whole cytoplasm
model.
...
water layer around each component, $l$, that yields such amount
...
number of water molecules in the system is satisfied.
...
$D_{cyto}$ to that in a dilute system, $D_{dil}$
...
larger. It's $D_{cyto}/D_{dil}$ is 0.07, indicating that it's movement
...
\caption{A) Diffusion coefficient of crowders from simulations under
  dilute conditions (blue bars) and cytoplasm simulations (orange
  bars). B) The ratio between diffusion coefficients obtained from
...
and FBP (Fig.~\ref{fig:tRNA_aggregation}). Thus, we decided to omit the tRNA
...
The structures of the individual chains of the crowders that were used
...
their C$\upalpha$ atoms, in the cytoplasm model increased by less than 11\% when compared to the dilute condition, except for
...
oligomers are not disturbed significantly in the cytoplasm model when
compared to the dilute condition. Similar to what can be seem from
the RMSD values, which reflect the structural integrity of individual
chains, the oligomer that undergoes the largest change is TufA
...
\caption{A) RMSD for all protein crowders from simulations under
  dilute conditions (blue bars) and in the cytoplasm model (orange
...
Root mean square fluctuation (RMSF) calculations show that, overall,
the shape of residue-wise flexibility profiles of the proteins is not
affected by crowding (Fig.~\ref{fig:rmsf}), which is in agreement with our results showing
that their structural integrity does not change significantly in the
cytoplasm model in most cases (Fig.~\ref{fig:structural_integrity_chain}A). 
Accordingly, we
can see significant changes in the shape of the flexibility profile
for regions that undergo conformational changes, such as the Switch I
$\upbeta$-hairpin of TufA around residues 40-62 and the flexible loop
of MetE around residues 450-460 (Fig.~\ref{fig:rmsf}).
...
the translational diffusion constant of tRNA being much lower than expected
...
eliminate this
problem~\cite{Best2014a,Piana2015a,Bashardanesh2018b,Carballo2018a}. 
Apart from
...
simulation box. This force field or similarly adjusted ones were also
...
condition  (Fig.~\ref{fig:structural_integrity_chain}B). This
...
complex~\cite{nissen1995crystal}). However, no TufA-tRNA complex is found
present in our simulation boxes within three times \SI{1}{\micro\second} of
simulations.
...
aggregated around Mg$^{2+}$ and, as a result, around tRNA.
...
of cytoplasm models. We found that protonating
...
FBPH$_4$ are not the biologically relevant forms of these molecules at a
...
environment are restrained to smaller volumes and shorter intermolecular distances, which mitigates the effect of charge
...
corresponds to applying an effective dielectric constant of about 1.7, alternatively scaling the charges down by $\sqrt{1.7}$ which would mean that FBP would get a charge of approximately -3 instead of -4.
...
metabolites and proteins. Maybe aggregation also contributed to 
slower diffusion found in that study but it was not evident during their
\SI{140}{\nano\second} simulation~\cite{Yu2016a}.
...
of 1.5 million atoms (Table \ref{tbl:soup_components}). The identities
and amounts of each component of the model were chosen based on
...
\ref{fig:structural_integrity_chain}.
...
to investigate the effect of crowding on it, to add new small- or macromolecular
 crowders, evaluate the effect of temperatures and change the
...
modest computational power it is possible, for example, to build many
...
The Swedish research council is acknowledged for a grant of computer
time (SNIC2018-2-42) through the High Performance Computing Center
North in Ume{\aa}, Sweden. Funding from eSSENCE - The e-Science
Collaboration (Uppsala-Lund-Ume{\aa}, Sweden) and FAPESP - S{\~a}o
Paulo Research Foundation (grants 2013/00927-7 and 2016/04958-2) are
